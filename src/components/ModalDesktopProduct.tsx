...
import { ModalDesktopProduct } from "../types";
...
export default function ModalDesktopProduct({
  ...
  return (
    <article className="flex flex-col items-center justify-center absolute inset-0 bg-black bg-opacity-75 z-[9999999] min-h-screen h-screen w-screen">
      <button className="ml-[29%] mb-4" onClick={onModalDesktop}>
        <CloseButton />
      </button>
      <div className="w-[30%]">
        <img
        ...
        <button
          onClick={handlePrev}
          className="absolute w-12 h-12 flex items-center justify-center pr-1 right-[63.3%] top-[35%] bg-white rounded-full sm:top-[50%] sm:translate-y-[-120%]"
        >
          <PreviousButton />
        </button>
        <button
          onClick={handleNext}
          className="absolute w-12 h-12 flex items-center justify-center left-[63.3%] top-[35%] bg-white rounded-full sm:top-[50%] sm:translate-y-[-120%] pl-[0.15rem]"
        >
          <NextButton />
        </button>
